<template>
    <div class="home-art-culture-category " v-if="artscontents?.length > 0">
        ...
        <div class="home-int-c-content flex flex-col gap-3">
            ...
            <NuxtLink :to="`/category/${artscontents[0]?.cat_slug}/${artscontents[0]?.content_id}`" class="flex flex-col gap-2 group">
            ...
            <div class="h-p-c-excpt flex flex-col">
                <!-- Loop Item -->
                <NuxtLink :to="`/category/${artscontent?.cat_slug}/${artscontent?.content_id}`" class=" border-b py-3" v-for="artscontent in artscontents.slice(1,5)"
