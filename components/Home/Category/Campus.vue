<template>
    <div class="home-campus-category " v-if="campuses?.length > 0">
        <div class="category-header border-b-4 border-b-[#3375af] my-3">
            <NuxtLink :to="`/${campuses[0]?.category?.cat_slug}`" class="flex gap-3 items-center">
        ...
        <div class="home-campus-c-content flex flex-col gap-3">
            ...
            <div class="h-p-c-excpt flex flex-col">
                <!-- Loop Item -->
                <NuxtLink :to="`${campus?.category?.cat_slug}/${campus?.content_id}`" class=" border-b py-3" v-for="campus in campuses.slice(1,5)"
...
const { data: dcampus } = await useFetch("/api/home/campuscontent", {
    method: 'GET'
})
