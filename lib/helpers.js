...
export const EnglishDate = () => {
    const getDateToday = new Intl.DateTimeFormat('bn-bd', { weekday: 'long', year: 'numeric', month: 'long', day: "numeric", timeZone: "GMT" })
    let t= 0;
    return getDateToday.format(new Date()).replace(/,/g, match => ++t === 2 ? ' ' : match).replace('জানুয়ারী', 'জানুয়ারি')
}
// ========== English Date ================= //
...
export const postCreatedDateWithTime = (date) => {
    ...
    // If date value has
    if (date) {
        return getDateTime.format(new Date(date)).replace('এ', '|').replace('PM', 'পিএম').replace('AM', 'এএম')
    }
}
// ================ Get Bangla Post Create Date ============== //
